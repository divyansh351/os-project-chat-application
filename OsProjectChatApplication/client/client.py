    ...
    def run(self):
        ...
        try:
            ...
            self.window.textEdit.append(formatResult(
                color="red", text="Failed to connect to server"))
            ...
            self.window.textEdit.append(formatResult(
                color="green", text="Connected to server %s" % (self.host)))
            ...
            for sock in read_sockets:
                if sock == client_socket:
                    try:
                        ...
                        if not data:
                            self.window.textEdit.append(formatResult(
                                color="red", text="Disconnected from server"))
                            self.window.pushButton.setEnabled(False)
                            ...
                            if msg != "":
                                self.window.textEdit.append(
                                    formatResult(color="#44dbe2", text=msg))
                    except:
                        pass
...
class PchatWindow(QMainWindow):
    def __init__(self, parent=None):
        ...
        matching_item = self.listWidget.findItems(
            "127.0.0.1:1100", Qt.MatchExactly)
