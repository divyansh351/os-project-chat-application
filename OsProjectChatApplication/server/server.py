...
class ServerWindow(QMainWindow):
        ...
        if update == True:
            ...
            for x in range(3):
                self.tableWidget.setItem(
                    currentRow, x, QTableWidgetItem(str(args[x])))
    ...
    def setupServer(self):
        ...
        self.textEdit.insertHtml(formatResult(
            color="blue", text="Setting up server !"))
        ...
        self.textEdit.append(formatResult(
            color="green", text="Server is ready"))

        self.server.listen(30)
        self.socket_list.append(self.server)
        self.textEdit.append(formatResult(
            color="green", text="Listening for clients..."))

        self.thread_event = Event()
        self.c_thread = Thread(target=self.handle_socket,
    ...
    def handle_socket(self, thread_event):
        while self.STATE and not thread_event.is_set():
            ...
            ready_input, ready_ouput, error = select.select(
                self.socket_list, self.outputs, [], 3)
                        ...
                        if data:
                            if self.rooms == {}:
                                if data.split()[0] not in self.commands:
                                    send(
                                        sock, "Create or Join a room.")
                                    send(
                                        sock, "Use -create command to create a new room.")
                                    send(sock, "Ex: -create hacking.")

                                else:
                                    if data.split()[0] == "-join" or data.split()[0] == "-leave":
                                        send(sock, "No rooms avaiable to %s" %
                                             data.split()[0])
                                ...
                                if not flag:
                                    if data.split()[0] not in self.commands:
                                        send(
                                            sock, "Join the room to chat!!!")
                                        send(sock, "Ex: -join room_name")

                                    elif data.split()[0] == "-leave":
                                        send(
                                            sock, "Can't leave without joining")
                                    ...
                                    if data.split()[0] == "-leave":
                                        self.leave_room(
                                            sock, user_name, room_name)
                                        for o in self.room_map[room_name]:
                                            self.send_encrypted(
                                                o.sock, "User %s left the room" % user_name, user_name)
                                        ...
                                        for o in self.room_map[room_name]:
                                            if o.sock != sock:
                                                self.send_encrypted(
                                                    o.sock, text, user_name)

                        else:
                            self.outputs.remove(sock)
                            address = self.get_address(sock)
                            user_name = self.get_just_name(sock)
                            msg = "User %d (%s) is offline" % (
                                sock.fileno(), user_name)
                            ...
                            self.updateTableWidget(
                                False, user_name, address[0], address[1])
                    except:
                        self.outputs.remove(sock)
                        self.socket_list.remove(sock)
